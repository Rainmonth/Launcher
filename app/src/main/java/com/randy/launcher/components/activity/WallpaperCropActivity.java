...
public class WallpaperCropActivity extends BaseActivity implements Handler.Callback {
                ...
                public void run() {
                    if (req == mCurrentLoadRequest) {
                        onLoadRequestComplete(req,
                                req.src.getLoadingState() == BitmapRegionTileSource.BitmapSource.State.LOADED);
                    } else {
    ...
    @Thunk void addReusableBitmap(TiledImageRenderer.TileSource src) {
        synchronized (mReusableBitmaps) {
    ...
    protected void onLoadRequestComplete(LoadRequest req, boolean success) {
        ...
        if (success) {
            TiledImageRenderer.TileSource oldSrc = mCropView.getTileSource();
    ...
    public final void setCropViewTileSource(BitmapRegionTileSource.BitmapSource bitmapSource, boolean touchEnabled,
    ...
    interface CropViewScaleProvider {
        float getScale(TiledImageRenderer.TileSource src);
    }
}
